...
public class ScarpSwim : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (isSwimming == true)
            {
                UnityEngine.Random.Range(GameScarpManager.instance.goldKoi.swimSpeed - 1, GameScarpManager.instance.goldKoi.swimSpeed + 1);
                gameObject.GetComponent<Animator>().SetBool("IsSwimming", true);
                transform.position += transform.forward * GameScarpManager.instance.goldKoi.swimSpeed * Time.deltaTime;
    ...
    void TurnLoRFunc()
    {
        ...
        {
            transform.Rotate(Vector3.up * Time.deltaTime * GameScarpManager.instance.goldKoi.rotSpeed);
            //Debug.Log("IsTurn" + isTurnRight);
        }
        if (isTurnRight == false && isTurnLeft == true)
        {
            transform.Rotate(Vector3.down * Time.deltaTime * GameScarpManager.instance.goldKoi.rotSpeed);
            //Debug.Log("IsTurn" + isTurnRight);
        }
    }
